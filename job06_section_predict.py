...
model = load_model('./models/news_section_classifier0.6991005539894104.h5')
...
print('\n[전체 결과]')
print(df[['titles', 'category', 'predict', 'OX']].to_string())

# 오답만 출력
wrong = df[df.OX == 0][['titles', 'category', 'predict']]
print(f'\n[오답 목록] {len(wrong)}건 / 전체 {len(df)}건')
print(wrong.to_string())

print(f'\nTop-2 Accuracy: {df.OX.mean():.4f}')