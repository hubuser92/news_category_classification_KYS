...
x_train = np.load('./data/x_train_wordsize12516.npy', allow_pickle=True)
y_train = np.load('./data/y_train_wordsize12516.npy', allow_pickle=True)
x_test = np.load('./data/x_test_wordsize12516.npy', allow_pickle=True)
y_test = np.load('./data/y_test_wordsize12516.npy', allow_pickle=True)
print(x_train.shape, y_train.shape, x_test.shape, y_test.shape)

model = Sequential()
model.add(Embedding(12516,300))
model.build(input_shape=(None, 26))
model.add(Conv1D(32,5,padding='same',activation='relu'))
model.add(MaxPooling1D(1))
model.add(LSTM(128,activation='tanh',return_sequences=True)) # GRU 써도 된다
